...
def _duplicate_notifications(glance_api_cfg, image_sync_cfg, conn, exchange):
    logging.info("Duplicate notifications method") 
    print "Duplicate notifications" 
    ...
    while True:
        ...
        print "Pull message from queue..."
        print("Message event type : %s " % msg.payload['event_type'])   
...
def _sync_images(glance_api_cfg, image_sync_cfg, conn, exchange):
    ...
    while True:
	    ...
	    print "su rcuser -c \"rsync -a -e 'ssh -o StrictHostKeyChecking=no' %s@%s:%s %s\"" % (image_sync_cfg['rsync_user'],
            ...
            os.system("su rcuser -c \"rsync -a -e 'ssh -o StrictHostKeyChecking=no' "
